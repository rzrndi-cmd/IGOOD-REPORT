 ...
 */
    ...
    window.deleteDevice = function (code) {
        ...
        queueRecordSupabaseDelete('devices', 'code', code);
        save(DB_KEYS.devices, load(DB_KEYS.devices).filter(d => d.code !== code));
        renderDeviceStock();
        renderActiveSaleForm();
        toast('Unit dihapus');
    };
    ...
    window.deleteAcc = function (code) {
        ...
        queueRecordSupabaseDelete('accessories', 'code', code);
        save(DB_KEYS.accessories, load(DB_KEYS.accessories).filter(a => a.code !== code));
        renderAccStock();
        renderActiveSaleForm();
        toast('Aksesoris dihapus');
    };
    ...
    function renderTechnicians() {
        const tbody = $('techniciansTableBody');
        if (!tbody) return;
        const techs = load(DB_KEYS.technicians);
        tbody.innerHTML = techs.length
            ? techs.map(t => `<tr>
                <td data-label="Nama">${esc(t.name)}</td>
                <td><button class="btn-del" onclick="deleteTech('${esc(t.name)}')"><i class="ri-delete-bin-6-line"></i></button></td>
            </tr>`).join('')
            : '<tr><td colspan="2" class="text-dim" style="text-align:center;">Belum ada</td></tr>';
    }

    window.deleteTech = function (name) {
        if (!confirm(`Hapus teknisi ${name}?`)) return;
        queueRecordSupabaseDelete('technicians', 'name', name);
        save(DB_KEYS.technicians, load(DB_KEYS.technicians).filter(t => t.name !== name));
        renderTechnicians();
        renderActiveSaleForm();
    ...
    function renderServiceOrders() {
                    ...
                    <div class="stock-card-body">
                        <div class="stock-grid-details">
                            ...
                            <div class="stock-detail-item">
                                ...
                                <span class="stock-detail-val">${esc(order.technician || '-')}</span>
                            </div>
                            <div class="stock-detail-item">
                                <span class="stock-detail-label">Modal Part</span>
                                <span class="stock-detail-val mono">${partCost ? fmtRp(partCost) : '-'}</span>
                            </div>
                            <div class="stock-detail-item">
                                <span class="stock-detail-label">Biaya Jasa</span>
                                <span class="stock-detail-val mono">${serviceFee ? fmtRp(serviceFee) : '-'}</span>
                            </div>
                            <div class="stock-detail-item">
                                <span class="stock-detail-label">Total Modal</span>
                                <span class="stock-detail-val mono font-bold text-primary">${totalModal ? fmtRp(totalModal) : '-'}</span>
                            </div>
                            <div class="stock-detail-item">
                ...
                <div class="stock-card-body">
                    <div class="stock-grid-details">
                        ...
                        <div class="stock-detail-item">
                            ...
                            <select id="serviceOrderTechnician-${esc(order.code)}" class="text-sm" style="width:100%; padding:6px; border-radius:6px; margin-top:2px;" onchange="saveServiceOrderTechnician('${esc(order.code)}')">
                                ${techOptions}
                            </select>
                        </div>
                        <div class="stock-detail-item">
        ...
        orders.forEach(order => {
            ...
            bindRpFormatter(`serviceOrderTechnicianCost-${order.code}`);
        });
    }
    ...
    window.updateServiceOrderTotalCostLive = function (code) {
        ...
        const legacyCost = cleanRp(legacyCostInput?.value);
        const total = (partCost + serviceFee) || legacyCost || 0;
    ...
    window.deleteServiceCatalog = function (code) {
        if (!confirm(`Hapus ${code}?`)) return;
        queueRecordSupabaseDelete('serviceCatalog', 'code', code);
        save(DB_KEYS.serviceCatalog, load(DB_KEYS.serviceCatalog).filter(s => s.code !== code));
        renderServiceCatalog();
        renderActiveSaleForm();
        toast('Service catalog dihapus');
    };
    ...
    function renderMonthlyDetail() {
        ...
        if (activeMonthlySubTab === 'all') {
            ...
            tbody.innerHTML = filteredTxs.map((tx, idx) => {
                ...
                return `
                    <tr>
                        ...
                        <td data-label="Nama Item" class="text-xs">${esc(tx.itemName)}</td>
            ...
            unitTxs.forEach((tx, txIdx) => {
                ...
                const model = unit?.model || tx.itemName || '-';
                ...
                for (let i = 0; i < N; i++) {
                    ...
                    if (i === 0) {
                        ...
                        rowHtml += `
                            ...
                            <td${rowspanAttr} class="text-xs" style="border: 1px solid var(--border-color, #e2e8f0); vertical-align: middle;">${esc(model)}</td>
        ...
        if (hpTbody) {
            if (top5HPs.length === 0) {
                ...
                hpTbody.innerHTML = top5HPs.map((item, index) => {
                    ...
                    if (index === 0) badgeClass = 'badge-warn';
                    else if (index === 1) badgeClass = 'badge-secondary';
                    return `<tr>
                        ...
                        <td class="mono font-bold text-sm" style="text-align: center; color: var(--primary);">${item.qty} unit</td>
                    </tr>`;
                }).join('');
            }
        }
            ...
            if (top5Accs.length === 0) {
                ...
                accTopTbody.innerHTML = top5Accs.map((item, index) => {
                    ...
                    return `<tr>
                        ...
                        <td class="text-sm font-semibold">${esc(item.name)}</td>
                        <td class="mono font-bold text-sm" style="text-align: center; color: var(--success);">${item.qty} pcs</td>
                    </tr>`;
                }).join('');
            }
        }
    ...
    function renderExpensesPanel() {
        ...
        tbody.innerHTML = expenses.map((e, i) => {
            ...
            return `<tr>
                ...
                <td><span class="badge" style="background:var(--primary-glow);color:var(--primary);font-size:10px;padding:2px 6px;border-radius:4px;">${esc(catLabel)}</span></td>
                <td class="text-sm">${esc(e.description || '-')}</td>
                <td class="mono" style="text-align:right;font-weight:600;">${fmtRp(e.amount)}</td>
                <td style="text-align:center;">
                    <button class="btn-icon" title="Hapus" onclick="deleteExpense('${esc(e.id)}')" style="color:var(--danger);font-size:14px;"><i class="ri-delete-bin-line"></i></button>
                </td>
            </tr>`;
        }).join('') + `<tr style="background:var(--bg-dim);font-weight:700;border-top:2px solid var(--border);">
            <td colspan="4" style="padding:8px;text-align:right;color:var(--text-dim);font-size:12px;">TOTAL BEBAN BULAN INI:</td>
            <td class="mono" style="text-align:right;padding:8px;color:var(--danger);">${fmtRp(totalExpense)}</td>
            <td></td>
        </tr>`;
    }

    window.deleteExpense = function(id) {
        if (!confirm('Hapus beban operasional ini?')) return;
        const expenses = loadExpenses().filter(e => e.id !== id);
        saveExpenses(expenses);
        queueRecordSupabaseDelete('expenses', 'id', id);
        renderExpensesPanel();
        toast('Beban dihapus');
    };
    ...
    window.deleteOtherCatalog = function (code) {
        if (!confirm(`Hapus item ${code}?`)) return;
        queueRecordSupabaseDelete('otherCatalog', 'code', code);
        save(DB_KEYS.otherCatalog, load(DB_KEYS.otherCatalog).filter(o => o.code !== code));
        renderOtherCatalog();
        renderActiveSaleForm();
        toast('Item catalog dihapus');
    };
                ...
                <div class="stock-card-body">
                    <div class="stock-grid-details">
                        ...
                        <div class="stock-detail-item">
                            ...
                            <span class="stock-detail-val font-bold text-primary">${esc(item.requestedItem || [item.brand, item.model].filter(Boolean).join(' ') || '-')}</span>
                        </div>
                        <div class="stock-detail-item">
                        ...
                        <div class="stock-detail-item">
                            ...
                            <span class="stock-detail-val font-bold text-success">${esc(fmtRp(item.dpAmount || 0))} (${esc(paymentLabel(item.paymentMethod))})</span>
                        </div>
                        <div class="stock-detail-item">
                            <span class="stock-detail-label">Unit Link</span>
                            <span class="stock-detail-val mono font-bold text-primary">${item.linkedUnitCode ? esc(item.linkedUnitCode) : '-'}</span>
                        </div>
                        <div class="stock-detail-item">
                            <span class="stock-detail-label">Modal Rp</span>
                            <div style="display:flex; align-items:center; gap:4px; margin-top:2px;">
                                <span style="font-size:11px; font-weight:700; color:var(--text-dim);">Rp</span>
                                <input type="text" inputmode="numeric" id="preorderCost-${esc(item.code)}" value="${item.cost ? Number(item.cost).toLocaleString('id-ID') : ''}" placeholder="Modal" style="padding:4px 6px; font-size:12.5px; border-radius:6px; flex:1; width:100%;" ${disabledAttr}>
                            </div>
                        </div>
                        <div class="stock-detail-item">
                            <span class="stock-detail-label">IMEI</span>
                            <input type="text" id="preorderImei-${esc(item.code)}" value="${esc(item.imei || '')}" placeholder="Ketik IMEI" style="padding:4px 6px; font-size:12.5px; border-radius:6px; width:100%; margin-top:2px;" ${disabledAttr}>
                        </div>
                        <div class="stock-detail-item">
                            <span class="stock-detail-label">Status Preorder</span>
                            <div style="margin-top:2px;">${statusSelectHtml}</div>
                        </div>
                    </div>
                    <div class="stock-card-actions">
                        ${status === 'Done' ? `<span class="badge badge-purple">Selesai (Lunas)</span>` : `
    ...
    function checkAndProcessPreorderReady(preorder, previousStatus) {
        if (preorder.status === 'Ready' && previousStatus !== 'Ready') {
            ...
            if (isDevice) {
                ...
                const generatedCodes = nextDeviceCodes('INT', preorder.storage || '', preorder.color || '', targetCategory, 1);
                const generatedCode = generatedCodes[0];
                ...
                toast(`Unit baru ${generatedCode} otomatis ditambahkan ke stok ready.`);
            }
        }
    }
    ...
    function bindAdminForms() {
        ...
        const IPHONE_MODELS_DATA = [
            ...
            { group: 'iPhone SE & Seri 8/7/6', items: ['iPhone SE 3', 'iPhone SE 2', 'iPhone 8 Plus', 'iPhone 8', 'iPhone 7 Plus', 'iPhone 7'] },
            { group: 'iPad Series', items: ['iPad Pro M4 13"', 'iPad Pro M4 11"', 'iPad Pro M2 11"', 'iPad Air M2 11"', 'iPad 10.9 Gen 10', 'iPad 10.2 Gen 9', 'iPad Mini 6'] }
        ];
        ...
        $('btnSyncMonthlyRecap')?.addEventListener('click', () => {
            pullAllFromSupabase()
    ...
    function getSalesRecapData(monthKey = activeRecapMonth, salesFilter = activeRecapSales, searchQuery = activeRecapSearch) {
        ...
        services.forEach(order => {
            const dateRef = order.processedDate || order.date || '';
            if (dateRef.startsWith(monthKey) && order.status !== 'masuk') {
                if (salesFilter && (order.technician || '').toLowerCase() !== salesFilter.toLowerCase()) return;
                
                const fee = Number(order.paymentAmount || order.inspectionFee || order.sell || 0);
                const cost = Number(order.technicianCost || order.cost || 0);
                const profit = fee - cost;

                serviceList.push({
                    id: order.id,
                    code: order.code || `SRV-${order.id}`,
                    date: dateRef,
                    customerName: order.customerName || '-',
                    customerPhone: order.customerPhone || '-',
                    device: `${order.brand || ''} ${order.model || ''}`.trim() || 'Perangkat',
                    ...
                    status: order.status === 'keluar' ? 'Selesai' : 'Cancel'
                });
            }
        });
        ...
        let orderJasaList = [];
        periodTxs.forEach(tx => {
            if (tx.category === 'jasa_icloud' || tx.category === 'jasa_imei') {
                const cost = Number(tx.cost) || 0;
                const sell = Number(tx.sell) || 0;
                const profit = sell - cost;
                orderJasaList.push({
                    id: tx.id,
                    date: tx.date,
                    category: tx.category === 'jasa_icloud' ? 'Jasa iCloud' : 'Jasa IMEI',
                    name: tx.itemName || (tx.category === 'jasa_icloud' ? 'Bypass / Unlock iCloud' : 'Registrasi / Unlock IMEI'),
                    code: tx.code || '-',
    ...
    function renderSalesRecapPanel() {
        ...
        if (srvContainer) {
            if (!data.serviceList.length) {
                srvContainer.innerHTML = '<div class="card p-lg text-center text-dim" style="background:#ffffff; border-radius:12px;">Tidak ada data servis selesai pada periode ini</div>';
            } else {
                srvContainer.innerHTML = data.serviceList.map(s => `
                    <div class="recap-item-row" onclick="openRecapDetailModal('service', '${esc(s.id)}')">
                        <div class="recap-item-left-icon" style="background: rgba(249,115,22,0.1); color: #ea580c;">
                        ...
                        <div class="recap-item-main">
                            ...
                            <div class="recap-item-meta">
                                ...
                                <span>•</span>
                                <span class="badge ${s.status === 'Selesai' ? 'badge-success' : 'badge-warning'}" style="padding: 2px 8px; border-radius: 6px; font-size: 11px;">${esc(s.status)}</span>
                            </div>
                        </div>
                        <div class="recap-item-figures">
                            <div class="recap-item-price">${fmtRp(s.fee)}</div>
                            <div class="recap-item-profit-badge profit-pos">Laba: ${fmtRp(s.profit)}</div>
                        </div>
                        <div class="recap-item-chevron">
                            <i class="ri-arrow-right-s-line"></i>
                        </div>
    ...
    function openRecapDetailModal(type, id) {
        const data = getSalesRecapData();
        ...
        let heroPrice = 'Rp 0';
        let heroProfit = 'Rp 0 (0%)';

        if (type === 'unit') {
            itemData = data.unitSales.find(u => String(u.id) === String(id));
            if (!itemData) return;
            title = itemData.model;
            subtitle = `<span class="badge" style="background:#e0f2fe; color:#0284c7; font-weight:700; font-size:10px; padding:1px 6px;">Unit HP</span> • Kode: <span class="mono">${esc(itemData.code)}</span> • ${fmtDate(itemData.date)}`;
            iconHtml = '<i class="ri-smartphone-line"></i>';
            iconBg = 'rgba(37,99,235,0.1)';
            iconColor = '#2563eb';
            heroPrice = fmtRp(itemData.sellPrice);
            heroProfit = `${fmtRp(itemData.trueNetProfit)} (${itemData.marginPct.toFixed(1)}%)`;

            $('modalRecapDetailItemContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Unit / Model:</span><span class="value font-bold">${esc(itemData.model)}</span></div>
                <div class="recap-detail-row"><span class="label">Kode Stok / IMEI:</span><span class="value mono font-bold" style="background:#f1f5f9; padding:1px 6px; border-radius:4px; font-size:10.5px;">${esc(itemData.code)}</span></div>
            ...
            $('modalRecapDetailFinancialContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Harga Jual:</span><span class="value font-mono font-bold" style="color:#0f172a;">${fmtRp(itemData.sellPrice)}</span></div>
                <div class="recap-detail-row"><span class="label">Modal HPP:</span><span class="value font-mono" style="color:#64748b;">- ${fmtRp(itemData.unitCost)}</span></div>
                <div class="recap-detail-row"><span class="label">Beban Bonus:</span><span class="value font-mono" style="color:${itemData.bonusCost > 0 ? '#d97706' : '#64748b'}; font-weight:700;">${itemData.bonusCost > 0 ? '- ' + fmtRp(itemData.bonusCost) : 'Rp 0'}</span></div>
                <div class="recap-detail-row" style="background:#ecfdf5; border-radius:4px; padding:3px 6px; margin-top:2px;"><span class="label" style="color:#065f46; font-weight:700;">Laba Riil:</span><span class="value font-mono font-bold" style="color:#059669;">${fmtRp(itemData.trueNetProfit)}</span></div>
            `;

            $('modalRecapDetailPaymentContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Metode Bayar:</span><span class="value font-bold">${esc(itemData.paymentDisplay)}</span></div>
                ${itemData.creditAgent ? `<div class="recap-detail-row"><span class="label">Leasing:</span><span class="value badge" style="background:#e0f2fe; color:#0284c7; font-weight:700; padding:1px 6px;">${esc(itemData.creditAgent)}</span></div>` : ''}
                ${itemData.paymentMethod === 'split' ? `
                    <div class="recap-detail-row"><span class="label">• Split Cash:</span><span class="value font-mono">${fmtRp(itemData.splitCash)}</span></div>
                    <div class="recap-detail-row"><span class="label">• Split TF:</span><span class="value font-mono">${fmtRp(itemData.splitTransfer)}</span></div>
                    <div class="recap-detail-row"><span class="label">• Split Kredit:</span><span class="value font-mono">${fmtRp(itemData.splitCredit)}</span></div>
                ` : ''}
            `;

            $('modalRecapDetailStaffContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Pembeli:</span><span class="value font-bold">${esc(itemData.buyerName || '-')}</span></div>
                <div class="recap-detail-row"><span class="label">WhatsApp:</span><span class="value font-mono">${esc(itemData.buyerPhone || '-')}</span></div>
                <div class="recap-detail-row"><span class="label">Petugas Sales:</span><span class="value badge" style="background:#e0f2fe; color:#0284c7; padding:2px 7px; border-radius:4px; font-weight:700;">${esc(itemData.salesName || '-')}</span></div>
            `;
            ...
            if (bonusBox && bonusListElem) {
                if (itemData.bonusList && itemData.bonusList.length) {
                    ...
                    bonusListElem.innerHTML = itemData.bonusList.map(b => `
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 4px 0; border-bottom: 1px dashed rgba(245,158,11,0.3);">
                            <span style="font-size: 11px; color: #78350f;"><i class="ri-gift-line"></i> <strong>${esc(b.name || b.model)}</strong> (${Number(b.quantity || b.qty) || 1} pcs)</span>
                            <span class="mono" style="font-weight: 700; color: #b45309; font-size: 11px;">Beban: ${fmtRp((Number(b.cost) || 0) * (Number(b.quantity || b.qty) || 1))}</span>
                        </div>
                    `).join('');
                } else {
            ...
            itemData = data.accSold.find(a => String(a.id) === String(id));
            if (!itemData) return;
            title = itemData.name;
            subtitle = `<span class="badge" style="background:#ede9fe; color:#7c3aed; font-weight:700; font-size:10px; padding:1px 6px;">Aksesoris</span> • Brand: ${esc(itemData.brand)} • ${fmtDate(itemData.date)}`;
            iconHtml = '<i class="ri-plug-line"></i>';
            iconBg = 'rgba(139,92,246,0.1)';
            iconColor = '#8b5cf6';
            heroPrice = fmtRp(itemData.sell);
            heroProfit = `${fmtRp(itemData.profit)} (${itemData.sell > 0 ? ((itemData.profit / itemData.sell) * 100).toFixed(1) : 0}%)`;

            $('modalRecapDetailItemContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Nama Item:</span><span class="value font-bold">${esc(itemData.name)}</span></div>
            ...
            $('modalRecapDetailFinancialContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Harga Jual:</span><span class="value font-mono font-bold">${fmtRp(itemData.sell)}</span></div>
                ...
                <div class="recap-detail-row" style="background:#ecfdf5; border-radius:4px; padding:3px 6px; margin-top:2px;"><span class="label" style="color:#065f46; font-weight:700;">Laba Bersih:</span><span class="value font-mono font-bold" style="color:#059669;">${fmtRp(itemData.profit)}</span></div>
            `;
            ...
            itemData = data.serviceList.find(s => String(s.id) === String(id));
            if (!itemData) return;
            title = itemData.device;
            subtitle = `<span class="badge" style="background:#fef3c7; color:#b45309; font-weight:700; font-size:10px; padding:1px 6px;">Jasa Servis</span> • Nota: <span class="mono">${esc(itemData.code)}</span> • ${fmtDate(itemData.date)}`;
            iconHtml = '<i class="ri-tools-line"></i>';
            iconBg = 'rgba(245,158,11,0.1)';
            iconColor = '#f59e0b';
            heroPrice = fmtRp(itemData.fee);
            heroProfit = `${fmtRp(itemData.profit)} (${itemData.fee > 0 ? ((itemData.profit / itemData.fee) * 100).toFixed(1) : 0}%)`;

            $('modalRecapDetailItemContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">No. Nota:</span><span class="value mono font-bold" style="background:#f1f5f9; padding:1px 6px; border-radius:4px; font-size:10.5px;">${esc(itemData.code)}</span></div>
                <div class="recap-detail-row"><span class="label">Perangkat:</span><span class="value font-bold">${esc(itemData.device)}</span></div>
                <div class="recap-detail-row"><span class="label">Keluhan:</span><span class="value">${esc(itemData.complaint)}</span></div>
                <div class="recap-detail-row"><span class="label">Tanggal:</span><span class="value">${fmtDate(itemData.date)}</span></div>
            `;

            $('modalRecapDetailFinancialContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Biaya Servis:</span><span class="value font-mono font-bold">${fmtRp(itemData.fee)}</span></div>
                <div class="recap-detail-row"><span class="label">Fee Teknisi:</span><span class="value font-mono" style="color:#64748b;">- ${fmtRp(itemData.cost)}</span></div>
                <div class="recap-detail-row" style="background:#ecfdf5; border-radius:4px; padding:3px 6px; margin-top:2px;"><span class="label" style="color:#065f46; font-weight:700;">Laba Bersih:</span><span class="value font-mono font-bold" style="color:#059669;">${fmtRp(itemData.profit)}</span></div>
            `;

            $('modalRecapDetailPaymentContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Metode:</span><span class="value font-bold">${esc(paymentLabel(itemData.paymentMethod))}</span></div>
                <div class="recap-detail-row"><span class="label">Status:</span><span class="value badge ${itemData.status === 'Selesai' ? 'badge-success' : 'badge-warning'}">${esc(itemData.status)}</span></div>
            `;

            $('modalRecapDetailStaffContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Pelanggan:</span><span class="value font-bold">${esc(itemData.customerName || '-')}</span></div>
                <div class="recap-detail-row"><span class="label">Teknisi:</span><span class="value badge" style="background:#fef3c7; color:#b45309; padding:2px 7px; border-radius:4px; font-weight:700;">${esc(itemData.technician || '-')}</span></div>
            `;

            if ($('modalRecapDetailBonusBox')) $('modalRecapDetailBonusBox').style.display = 'none';
        } else if (type === 'order-jasa') {
            itemData = data.orderJasaList.find(j => String(j.id) === String(id));
            if (!itemData) return;
            title = itemData.name;
            subtitle = `<span class="badge" style="background:#e0f2fe; color:#0284c7; font-weight:700; font-size:10px; padding:1px 6px;">Order Jasa</span> • ID: <span class="mono">${esc(itemData.code)}</span> • ${fmtDate(itemData.date)}`;
            ...
            $('modalRecapDetailItemContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Nama Jasa:</span><span class="value font-bold">${esc(itemData.name)}</span></div>
                <div class="recap-detail-row"><span class="label">Kode ID:</span><span class="value mono font-bold" style="background:#f1f5f9; padding:1px 6px; border-radius:4px; font-size:10.5px;">${esc(itemData.code)}</span></div>
            ...
            $('modalRecapDetailFinancialContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Biaya Jasa:</span><span class="value font-mono font-bold">${fmtRp(itemData.sell)}</span></div>
            ...
            $('modalRecapDetailPaymentContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Metode Bayar:</span><span class="value font-bold">${esc(paymentLabel(itemData.paymentMethod))}</span></div>
            `;

            $('modalRecapDetailStaffContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Pelanggan:</span><span class="value font-bold">${esc(itemData.buyerName || '-')}</span></div>
                <div class="recap-detail-row"><span class="label">WhatsApp:</span><span class="value font-mono">${esc(itemData.buyerPhone || '-')}</span></div>
                <div class="recap-detail-row"><span class="label">Petugas Sales:</span><span class="value badge" style="background:#e0f2fe; color:#0284c7; padding:2px 7px; border-radius:4px; font-weight:700;">${esc(itemData.salesName || '-')}</span></div>
            `;
            ...
            itemData = data.otherList.find(o => String(o.id) === String(id));
            if (!itemData) return;
            title = itemData.name;
            subtitle = `<span class="badge" style="background:#f1f5f9; color:#475569; font-weight:700; font-size:10px; padding:1px 6px;">Lain-lain</span> • Kode: <span class="mono">${esc(itemData.code)}</span> • ${fmtDate(itemData.date)}`;
            ...
            $('modalRecapDetailItemContent').innerHTML = `
                <div class="recap-detail-row"><span class="label">Nama Barang:</span><span class="value font-bold">${esc(itemData.name)}</span></div>
                <div class="recap-detail-row"><span class="label">Kode ID:</span><span class="value mono font-bold" style="background:#f1f5f9; padding:1px 6px; border-radius:4px; font-size:10.5px;">${esc(itemData.code)}</span></div>
                <div class="recap-detail-row"><span class="label">Tanggal:</span><span class="value">${fmtDate(itemData.date)}</span></div>
            `;
        ...
        if ($('modalRecapDetailHeroProfit')) $('modalRecapDetailHeroProfit').textContent = heroProfit;
    ...
    async function exportSalesRecapPdf(isFullReport = false) {
        ...
        try {
            ...
            if (shouldExportServices && data.serviceList.length) {
                ...
                doc.autoTable({
                    startY: startTableY,
                    head: [['No', 'Nota', 'Tgl', 'Pelanggan', 'Unit & Kerusakan', 'Teknisi', 'Biaya Servis', 'Modal/Fee', 'Laba', 'Status']],
                    body: srvRows,
                    theme: 'grid',
                    styles: { fontSize: 8, cellPadding: 4 },
                    ...
                    columnStyles: {
                        ...
                        8: { halign: 'right', cellWidth: 65, textColor: [22, 163, 74] }
                    },
                    margin: { left: 40, right: 40 }
                });
