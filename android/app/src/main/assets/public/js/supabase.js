 ...
 */
    ...
    function mapDeviceForSupabase(device) {
        ...
        // Only include imei if the column exists in Supabase
        // To enable: run SQL: ALTER TABLE igood_device_stock ADD COLUMN IF NOT EXISTS imei TEXT DEFAULT '';
    ...
    function mapTransactionForSupabase(tx, options = {}) {
        ...
        const mapped = {
            ...
            updated_at: new Date().toISOString(),
            jasa_metadata: tx.jasaMetadata || ((tx.category && (tx.category.startsWith('order_jasa') || tx.category === 'order_jasa_icloud')) ? {
    ...
    async function pullTableFromSupabase(tableKey, mapFn, dbKey) {
        try {
            ...
            if (rows && Array.isArray(rows)) {
                ...
                if (dbKey === DB_KEYS.serviceOrders) saveServiceOrders(mapped);
                else if (dbKey === DB_KEYS.preorders) savePreorders(mapped);
                else if (dbKey === DB_KEYS.transactions) saveTransactions(mapped);
                else if (dbKey === DB_KEYS.expenses) saveExpenses(mapped);
                else save(dbKey, mapped);
            }
